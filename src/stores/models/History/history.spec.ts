...
const getStore = (
  historySnap: IHistoryStore = defaultHistoryStore,
  canvasSnap: ICanvasStore = defaultCanvasStore
) =>
  model.create({
    canvas: canvasSnap,
    history: historySnap,
...
const getCanvasSnap = (snap: ICanvasStore = defaultCanvasStore) =>
  getSnapshot(CanvasStore.create(snap));
  ...
  describe('Actions', () => {
    describe('addSnapShot', () => {
      it('should add snap success', () => {
        ...
        const expectResult: IHistoryStore = {
          snapShots: [defaultCanvasStore, defaultCanvasStore],
          activeSnapIndex: 1,
        };
    ...
    describe('changeIndexBy', () => {
      it('should apply snap to canvas store', () => {
        const store = getStore(
          {
            snapShots: [{ figures: [] }, { figures: [{ ...defaultFigure }] }],
            activeSnapIndex: 0,
          },
          {
            figures: [],
      ...
      it('should change active index', () => {
        const store = getStore({
          snapShots: [getCanvasSnap(), getCanvasSnap()],
          activeSnapIndex: 1,
        });
        store.history.changeIndexBy(-1);
      ...
      it('should not apply for wrong index', () => {
        ...
        const store = getStore({
          snapShots: [getCanvasSnap(), sampleCanvasStore],
          activeSnapIndex: 1,
        });
        store.history.changeIndexBy(1);
